...
mod tests {
    ...
    use solana_program::{
        hash::hash, instruction::AccountMeta, instruction::Instruction, system_program,
    };
    use solana_sdk::{
        message::Message, pubkey::Pubkey, signature::Keypair, signer::Signer,
        signer::keypair::read_keypair_file, system_instruction::transfer, transaction::Transaction,
    };
    use std::str::FromStr;
    ...
    fn keygen() {
        ...
        println!("Copy the following as json file");
        println!("{:?}", kp.to_bytes());
    }
    ...
    fn transfer_sol() {
        ...
        let message = "I verify my Solana Keypair!";
        let sig = keypair.sign_message(message.as_bytes());

        match sig.verify(&pubkey.to_bytes(), message.as_bytes()) {
            true => println!("Signature verified"),
            false => println!("Verification failed"),
        }

        let to_pubkey = Pubkey::from_str("C8MxGwhe53cdKJuTvQ9rHz5swH4vdWULZtyc1Hqnkbgn").unwrap();
    ...
    fn enroll() {
        ...
        let (prereq_pda, _account_bump) =
            Pubkey::find_program_address(account_seeds, &turbin3_prereq_program);
        let authority_seeds = &[
            [99, 111, 108, 108, 101, 99, 116, 105, 111, 110].as_ref(),
            collection.as_ref(),
        ];
